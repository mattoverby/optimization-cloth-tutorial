...
class Solver
{
  public:
    ...
    double gradient(const ClothMesh& cloth,
                    ...
                    Eigen::MatrixXd& grad) const
    {
        ...
        // Momentum potential
        tot_energy += objective.momentum_gradient(xk, x_start, v, cloth.masses, dt, grad);

        // Stretch springs
        int num_edges = cloth.E.rows();
        for (int i = 0; i < num_edges; ++i) {
            Eigen::RowVector2i e = cloth.E.row(i);
            tot_energy += objective.spring_gradient(xk, e[0], e[1], cloth.E0[i], spring_k, grad);
        }
        ...
        for (int i = 0; i < num_bend; ++i) {
            ...
            tot_energy += objective.spring_gradient(xk, b[0], b[1], cloth.B0[i], bend_k, grad);
        }

        // Collision springs
        int num_verts = xk.rows();
        for (int i = 0; i < num_verts; ++i) {
            tot_energy += objective.collision_gradient(xk, i, collision_k, grad);
        }
